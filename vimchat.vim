...
com! VimChatSetStatus py VimChat.setStatus()
...
class VimChatScope:
    ...
    class OtrOps:
        ...
        def write_fingerprints(self, opdata=None):
            fpath = os.path.expanduser(
                self.otr_basedir + '/' + self.otr_fingerprints)
            if accountname in VimChat.accounts.keys(): 
                otr.otrl_privkey_write_fingerprints(
                    VimChat.accounts[accountname]._otr_userstate, fpath)
            else:
                print "User: " + accountname + " not connected"
        #}}}
        #{{{ gone_secure
        def gone_secure(self, opdata=None, context=None):
            ...
            if buf:
                VimChat.appendStatusMessage( buf,"[OTR]",
        ...
        def account_name(
            ...
            if accountname in VimChat.accounts.keys(): 
                jid = VimChat.accounts[accountname]._jid.split('/')[0]
    ...
    class JabberConnection(threading.Thread):
        ...
        def jabberPresenceReceive(self, conn, msg):
            ...
            if type == "groupchat":
                ...
                VimChat.presenceUpdate(str(fromJid), fromJid,show,status,priority)
        #}}}
        ...
        def otrSetup(self):
            ...
            keypath = os.path.expanduser(
                self._parent.otr_basedir + '/' + self._parent.otr_keyfile)

            #Make the otr directory
            basedir = os.path.expanduser(self._parent.otr_basedir)
            ...
            fprintPath = os.path.expanduser(
                self._parent.otr_basedir + '/' + self._parent.otr_fingerprints)
        ...
        def otrManualVerifyBuddy(self, jid):
            ...
            if buf:
                VimChat.appendStatusMessage( buf,"[OTR]","-- Verified Fingerprint of " + jid)
                print "Verified "+jid
        #}}}
        #{{{ otrSMPVerifyBuddy
        ...
        def otrSMPVerifyFailed(self,context):
            ...
            buf = VimChat.beginChat(cantext.accountname, jid)
            if buf:
                VimChat.appendStatusMessage( buf,"[OTR]",
    ...
    #}}}
    #{{{ appendMessage
    def appendMessage(self, buf, message, showJid='Me',secure=False):
        if not buf:
    ...
    #}}}
    #{{{ appendStatusMessage
    def appendStatusMessage(self, buf, prefix, message):
        if not buf:
            print "VimChat: Invalid Buffer to append to!"
            return 0
    ...
    def deleteChat(self):
        ...
        del self.accounts[account]._chats[vim.current.buffer.name.split('/')[-1]]
    ...
    def openGroupChat(self):
        ...
        buf = VimChat.beginChat(self,_jids, chatroom, True)
        vim.command('sbuffer ' + str(buf.number))
        VimChat.toggleBuddyList()
        vim.command('wincmd K')
    ...
    def openLogFromBuddyList(self):
        jid = VimChat.getBuddyListItem('jid')
        VimChat.openLog(jid)
    #}}}
    #{{{ openLogFromChat
    ...
    def sendMessage(self):
        ...
        if not self.isGroupChat():
            VimChat.appendMessage(chatBuf,body,'Me',secure)
    ...
    def messageReceived(self, account, fromJid, message, secure=False, groupChat=""):
        ...
        if groupChat == "":
            ...
            VimChat.appendMessage(buf, message, fromJid, secure)
        else:
    ...
    def otrSMPRequestNotify(self, jid, question):
        ...
        if buf:
            ...
            VimChat.appendMessage(buf,message, "[OTR]")
